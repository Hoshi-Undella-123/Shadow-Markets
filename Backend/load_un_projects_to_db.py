"""
Loads UN/SDG projects from JSON into the database, mapping to the unified Project model.
"""
...
def load_un_projects(json_path="un_projects.json"):
    ...
    with engine.connect() as conn:
        for p in projects:
            mapped_project = {
                ...
                'project_type': p.get('project_type', 'UN'),
                'sectors': p.get('tags', []),
                'source': 'UN',
            ...
            try:
                ...
                print(f"Error loading UN project {p.get('id')}: {e}")
                continue
        conn.commit()
    print(f"Loaded {count} UN projects into database.")
